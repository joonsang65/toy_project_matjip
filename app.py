                ...
                if selected_quick != "선택하세요":
                    with st.spinner("추천 음식점을 찾고 있습니다..."):
                        ...
                        if recommendations:
                            ...
                            st.session_state.recommendation_history.append((selected_quick, recommendations))
                        else:
                            st.warning("추천 결과를 찾지 못했습니다. 다른 키워드로 시도해보세요.")

        with col2:
            st.subheader("🔍 상세 추천")
            cuisine_type = st.selectbox("선호하는 음식 종류", ["전체"] + sorted(df['업종'].dropna().unique().tolist()))
            ...
            custom_request = st.text_area("추가 요청사항 (선택사항)", placeholder="예: 특정 음식 종류, 분위기, 원하는 조건 등")
            ...
            if st.button("상세 추천 받기", key="detailed_rec"):
                ...
                with st.spinner("맞춤 추천을 생성하고 있습니다..."):
                    ...
                    if recommendations:
                        ...
                        st.markdown(recommendations)
                        st.session_state.recommendation_history.append((query, recommendations))
                    else:
